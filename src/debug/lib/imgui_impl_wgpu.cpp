...
//  2024-01-22: (Breaking) ImGui_ImplWGPU_Init() now takes a ImGui_ImplWGPU_InitInfo structure instead of variety of
//  parameters, allowing for easier further changes. 2024-01-22: Fixed pipeline layout leak. (#7245) 2024-01-17:
//  Explicitly fill all of WGPUDepthStencilState since standard removed defaults. 2023-07-13: Use
//  WGPUShaderModuleWGSLDescriptor's code instead of source. use WGPUMipmapFilterMode_Linear instead of
//  WGPUFilterMode_Linear. (#6602) 2023-04-11: Align buffer sizes. Use WGSL shaders instead of precompiled SPIR-V.
//  2023-04-11: Reorganized backend to pull data from a single structure to facilitate usage with multiple-contexts (all
//  g_XXXX access changed to bd->XXXX). 2023-01-25: Revert automatic pipeline layout generation (see
//  https://github.com/gpuweb/gpuweb/issues/2470) 2022-11-24: Fixed validation error with default depth buffer settings.
//  2022-11-10: Fixed rendering when a depth buffer is enabled. Added 'WGPUTextureFormat depth_format' parameter to
//  ImGui_ImplWGPU_Init(). 2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11. 2021-11-29: Passing
//  explicit buffer sizes to wgpuRenderPassEncoderSetVertexBuffer()/wgpuRenderPassEncoderSetIndexBuffer(). 2021-08-24:
//  Fixed for latest specs. 2021-05-24: Add support for draw_data->FramebufferScale. 2021-05-19: Replaced direct access
//  to ImDrawCmd::TextureId with a call to ImDrawCmd::GetTexID(). (will become a requirement) 2021-05-16: Update to
//  latest WebGPU specs (compatible with Emscripten 2.0.20 and Chrome Canary 92). 2021-02-18: Change blending equation
//  to preserve alpha in output buffer. 2021-01-28: Initial version.
...
static void ImGui_ImplWGPU_SetupRenderState(ImDrawData* draw_data, WGPURenderPassEncoder ctx, FrameResources* fr)
{
  ...
  // (bottom right).
  {
    ...
    wgpuQueueWriteBuffer(
        bd->defaultQueue, bd->renderResources.Uniforms, offsetof(Uniforms, MVP), mvp, sizeof(Uniforms::MVP));
    float gamma;
    ...
    wgpuQueueWriteBuffer(
        bd->defaultQueue, bd->renderResources.Uniforms, offsetof(Uniforms, Gamma), &gamma, sizeof(Uniforms::Gamma));
  }
  ...
  wgpuRenderPassEncoderSetIndexBuffer(ctx,
                                      fr->IndexBuffer,
