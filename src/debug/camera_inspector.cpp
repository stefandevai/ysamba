...
void CameraInspector::update()
{
  ...
  if (ImGui::Begin("Camera Inspector", &open, ImGuiWindowFlags_NoFocusOnAppearing))
  {
    ...
    ImGui::Text(
        "m_size: (%s, %s)", std::to_string(m_camera.m_size.x).c_str(), std::to_string(m_camera.m_size.y).c_str());
