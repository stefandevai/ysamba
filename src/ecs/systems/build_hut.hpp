...
class BuildHutSystem
{
 ...
 private:
  ...
  void m_preview_hut_target(const Vector3i& position, const uint32_t hut_size, entt::registry& registry);
  void m_create_hut_job(const Vector3i& position, const uint32_t hut_size, entt::registry& registry);
