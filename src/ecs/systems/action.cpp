...
/* void ActionSystem::m_update_inspector_content(const entt::entity entity, */
...
/*   /1* const auto& position = registry.get<Position>(entity); *1/ */
/*   /1* text.set_text(agent.name + " (" + std::to_string(position.x) + ", " + std::to_string(position.y) + ")"); *1/ */
/* } */
/* } */

/* void ActionSystem::m_destroy_inspector(entt::registry& registry) */
/* { */
/*   registry.destroy(m_inspector_quad); */
/*   registry.destroy(m_inspector_text); */
/* } */

/* bool ActionSystem::m_is_valid(const entt::registry& registry) const */
/* { */
