...
TextureLoader::TextureLoader(const std::string& filepath, const int horizontal_frames, const int vertical_frames)
    : AssetLoader(AssetType::TEXTURE),
      m_filepath(filepath),
...
std::shared_ptr<Asset> TextureLoader::construct()
{
  ...
  {
    return std::make_shared<Texture>(m_filepath);
  }
  break;

  case TextureMode::UNIFORM_ATLAS:
  {
    const auto texture = std::make_shared<Texture>(m_filepath, m_horizontal_frames, m_vertical_frames);
