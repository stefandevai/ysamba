...
TempGenerator::TempGenerator(const Vector3i& size) : size(size)
{
  m_load_params(m_default_params_filepath);
}

void TempGenerator::generate(const int seed)
{
  spdlog::info("=============================");
...
void TempGenerator::set_size(const Vector3i& size)
{
  this->size = size;
}

void TempGenerator::m_load_params(const std::string& filepath)
{
  JSON json{filepath};
...
void TempGenerator::m_compute_maps(const int seed)
{
...
void TempGenerator::m_generate_biomes()
{
  ...
  for (int j = 0; j < size.y; ++j)
  {
    for (int i = 0; i < size.x; ++i)
    {
      ...
      if (height_value > 0.137 && height_value < 0.196 && humidity_value >= 0.1f
          && humidity_value < 0.4f)
      {
...
void TempGenerator::m_flood_fill(std::vector<int>& grid, const int x, const int y, const int value_to_fill)
{
...
bool TempGenerator::m_valid_coordinate(const int x, const int y)
{
  return !(x < 0 || y < 0 || x >= size.x || y >= size.y);
}

std::vector<IslandData> TempGenerator::m_get_islands(std::vector<int>& grid, const uint32_t islands_to_keep)
{
...
IslandData TempGenerator::m_get_island(const std::vector<int>& grid, std::vector<int>& mask, const int x, const int y)
{
