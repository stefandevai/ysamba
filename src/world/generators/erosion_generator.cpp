...
std::vector<IslandData> ErosionGenerator::m_get_islands(std::vector<int>& tiles)
{
...
IslandQueue ErosionGenerator::m_get_island_queue(const std::vector<int>& tiles)
{
...
IslandData ErosionGenerator::m_get_island(const std::vector<int>& tiles,
                                          std::vector<int>& mask,
                                          const int x,
                                          const int y)
{
...
bool ErosionGenerator::m_valid_coord(const int x, const int y)
{
  return !(x < 0 || y < 0 || x >= m_width || y >= m_height);
}

bool ErosionGenerator::m_valid_point(const Point<int>& point)
{
  return !(point.x < 0 || point.y < 0 || point.x >= m_width || point.y >= m_height);
}

std::vector<Point<int>> ErosionGenerator::m_get_coastline(const std::vector<int>& tiles, const IslandData& island)
{
...
bool ErosionGenerator::m_contains_island_area(const Point<int>& point,
...
void ErosionGenerator::m_flood_fill(const int value, const int x, const int y, std::vector<int>& tiles)
{
...
Point<int> ErosionGenerator::m_get_first_coastline_point(const std::vector<int>& tiles, const IslandData& island)
{
...
int ErosionGenerator::m_get_point_value(const Point<int>& point, const std::vector<int>& tiles)
{
  return tiles[point.y * m_width + point.x];
}

bool ErosionGenerator::m_is_coast_point(const Point<int>& point, const std::vector<int>& tiles)
{
  // If point is water
...
bool ErosionGenerator::m_is_water(const Point<int>& point, const std::vector<int>& tiles)
{
  return m_valid_point(point) && m_get_point_value(point, tiles) == TileType::Water;
}

std::vector<Point<int>> ErosionGenerator::m_get_bays(std::vector<Point<int>>& coastline, const IslandData& island)
{
  ...
  for (std::size_t n = 0; n < coastline.size(); ++n)
  {
    ...
    for (std::size_t m = 0; m < coastline.size(); ++m)
    {
      ...
      {
        const auto bay_data =
            m_get_bay_data(*coast_point_a, *coast_point_b, island.mask, minimum_area, area_mask, TileType::Water);
...
bool ErosionGenerator::m_has_land_intersection(const Point<int>& point_a,
...
BayData ErosionGenerator::m_get_bay_data(const Point<int>& point_a,
                                         const Point<int>& point_b,
                                         const std::vector<int>& tiles,
                                         const int minimum,
                                         std::vector<int>& mask,
...
void ErosionGenerator::m_build_island_structure(IslandData& island)
{
  ...
  for (const auto& site : diagram.get_sites())
  {
    ...
    while (half_edge != nullptr)
    {
      if (half_edge->origin != nullptr && half_edge->destination != nullptr)
      {
        ...
        if (island.mask[origin.y * m_width + origin.x] == TileType::Water ||
            island.mask[destination.y * m_width + destination.y] == TileType::Water)
        {
          is_coast = true;
        }
      }
...
bool ErosionGenerator::m_center_is_coast(const Point<int>& center, const std::vector<int>& island_mask)
{
...
void ErosionGenerator::m_generate_main_river(IslandData& island,
                                             std::vector<Point<int>>& bays,
...
std::pair<Point<int>, Point<int>> ErosionGenerator::m_get_river_source_and_mouth(IslandData& island,
                                                                                 std::vector<Point<int>>& bays,
                                                                                 const int seed)
{
...
RiverData ErosionGenerator::m_get_river_data(const Point<int>& source, const Point<int>& mouth)
{
  RiverData river;
...
void ErosionGenerator::m_create_meanders(RiverData& river,
...
double ErosionGenerator::m_distance(const Point<double>& point_a, const Point<double>& point_b)
{
...
float ErosionGenerator::m_distance(const Point<int>& point_a, const Point<int>& point_b)
{
...
double ErosionGenerator::m_menger_curvature(const Point<double>& point_a,
...
void ErosionGenerator::m_draw_point(const Point<int>& point, const int value, std::vector<int>& tiles)
{
  tiles[point.y * m_width + point.x] = value;
}

void ErosionGenerator::m_draw_big_point(const Point<int>& point, const int value, std::vector<int>& tiles)
{
  tiles[point.y * m_width + point.x] = value;
...
void ErosionGenerator::m_draw_line(const Point<int>& origin,
